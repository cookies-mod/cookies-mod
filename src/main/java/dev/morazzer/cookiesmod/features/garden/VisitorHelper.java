...
public class VisitorHelper implements Module {
	...
	private List<Text> createMaterialList(Ingredient rootIngredient, int deep, ArrayList<Ingredient> visited) {
		...
		for (Ingredient ingredient : ingredientListSorted) {
			...
			if (item == null) {
				texts.add(Text.literal(StringUtils.leftPad("", deep))
						.append(" -> ")
						.append(Text.literal("Can not find item with internal id " + finalIngredient).formatted(Formatting.RED))
						.formatted(Formatting.GRAY));
				continue;
			}

			MutableText text = Text
					.literal(StringUtils.leftPad("", deep))
					.append(" -> ").formatted(Formatting.DARK_GRAY);
