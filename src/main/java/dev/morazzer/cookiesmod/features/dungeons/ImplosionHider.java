...
/**
 * Hides big explosion particles near players with a wither impact weapon.
 */
...
public class ImplosionHider implements Module {
    ...
    public void load() {
        ExplosionLargeParticleCallback.EVENT.register((world, x, y, z, d, spriteProvider) -> {
            if (!ConfigManager.getConfig().dungeonCategory.implosionHider.getValue()) {
                return true;
            }
            ...
            try {
                for (PlayerEntity playerEntity : Optional.ofNullable(MinecraftClient.getInstance().world)
                    .map(ClientWorld::getPlayers).orElse(Collections.emptyList())) {
