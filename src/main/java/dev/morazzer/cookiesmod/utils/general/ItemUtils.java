...
public class ItemUtils {
	...
	public static boolean doesCurrentItemHaveEnchantments(String... enchantments) {
		return getNbtFromMainHand()
				.map(nbt -> nbt.getCompound("ExtraAttributes"))
				.map(extraAttributes -> extraAttributes.getCompound("enchantments"))
				.map(enchantmentsContainer -> {
	...
	public static boolean hasSkyblockItemInMainHand() {
		return Optional.ofNullable(MinecraftClient.getInstance())
				...
				.flatMap(ItemUtils::getSkyblockId).isPresent();
	}
