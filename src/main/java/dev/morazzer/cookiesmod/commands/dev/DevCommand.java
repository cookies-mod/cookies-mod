...
import dev.morazzer.cookiesmod.data.player.PlayerData;
        ...
        return Helper.literal("dev")
				...
				// endregion
                        ...
                        .then(Helper.literal("load").executes(context -> {
                            ...
                            timer.start();
                            context.getSource().sendFeedback(CookiesMod.createPrefix(0xFFFFFF).append("Loading player data... "));
                            Optional<PlayerData> playerData = PlayerStorage.getPlayerData();
                            timer.stop();
                            context.getSource().sendFeedback(CookiesMod.createPrefix(0xFFFFFF).append(playerData.toString()));
                            context.getSource().sendFeedback(CookiesMod.createPrefix(0xFFFFFF).append("Loaded player data, took %s".formatted(timer.elapsed())));

                            return Command.SINGLE_SUCCESS;
                        }))
                        .then(Helper.literal("store").then(Helper.argument("test", StringArgumentType.string()).executes(context -> {
                            String argument = context.getArgument("test", String.class);
                            if (PlayerStorage.getPlayerData().isEmpty()) {
                                context.getSource().sendFeedback(CookiesMod.createPrefix(0xFF).append("Player is empty"));
                                return -1;
                            }
                            PlayerStorage.getPlayerData().get().setTest(argument);
                            context.getSource().sendFeedback(CookiesMod.createPrefix(0xFFFFFF).append("Set test to %s".formatted(argument)));
                            PlayerStorage.savePlayerData();
                            return Command.SINGLE_SUCCESS;
                        })))
                        .then(Helper.literal("print_test").executes(context -> {
                            Optional<PlayerData> playerData = PlayerStorage.getPlayerData();

                            context.getSource().sendFeedback(CookiesMod.createPrefix(0xFFFFFF).append("Test is %s".formatted(playerData.map(PlayerData::getTest).orElse("<empty>"))));

                            return Command.SINGLE_SUCCESS;
                        }))
                ).then(Helper.literal("profile")
                        .then(Helper.literal("load")
                                .executes(context -> {
                                    Optional<ProfileData> profileData = ProfileStorage.getCurrentProfile();
                                    if (profileData.isEmpty()) {
                                        return Command.SINGLE_SUCCESS;
                                    }
                                    context.getSource().sendFeedback(CookiesMod.createPrefix(0xababab).append(Text.literal("Loaded profile").styled(style -> style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Text.literal(profileData.get().toString()))))));


                                    return Command.SINGLE_SUCCESS;
                                }))
                        .then(Helper.literal("get_gamemode").executes(context -> {
                            context.getSource().sendFeedback(CookiesMod.createPrefix(0xff00ff).append("Gamemode: %s".formatted(ProfileStorage.getCurrentProfile().map(ProfileData::getGameMode).orElse(GameMode.UNSET))));
                            return Command.SINGLE_SUCCESS;
                        })));
    }
