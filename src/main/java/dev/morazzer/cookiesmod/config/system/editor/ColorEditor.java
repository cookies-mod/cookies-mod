...
public class ColorEditor extends ConfigOptionEditor<Color, ColorOption> {
	...
	public void init() {
		...
		this.textFieldWidget.setRenderTextProvider((textFieldString, integer) -> {
			...
			return Text.literal("#").append(s).append(Text.literal(textFieldString).formatted(Formatting.WHITE)).formatted(Formatting.WHITE).asOrderedText();
		});
	}
	...
	public void renderOverlay(DrawContext context, int mouseX, int mouseY, float delta, int optionWidth) {
		...
		RenderUtils.renderRectangle(context, overlayX, overlayY, overlayWidth, 89, true);

		Color value = this.option.getValue();
		context.setShaderColor(value.getRed() / 255f, value.getGreen() / 255f, value.getBlue() / 255f, 1);
		context.drawTexture(COLOR_SATURATION, overlayX + 75, overlayY + 5, 0, 0, 10, 64, 10, 64);

		if (this.option.isAllowAlpha()) {
			context.setShaderColor(value.getRed() / 255f, value.getGreen() / 255f, value.getBlue() / 255f, 1);
			//context.drawTexture(COLOR_OPACITY, overlayX + 90, overlayY + 5, 0, 0, 10, 64, 10, 64);
			context.fillGradient(overlayX + 90, overlayY + 6, overlayX + 100, overlayY + 69, 0xFFFFFFFF, 0x00FFFFFF);
			context.fill(overlayX + 91, overlayY + 5, overlayX + 98, overlayY + 7, 0xFFFFFFFF);
		}
		context.setShaderColor(1, 1, 1, 1);

		context.drawTexture(COLOR_BAR_OVERLAY, overlayX + 75, overlayY + 5, 0, 0, 10, 64, 10, 64);
		if (this.option.isAllowAlpha()) {
			context.drawTexture(COLOR_BAR_OVERLAY, overlayX + 90, overlayY + 5, 0, 0, 10, 64, 10, 64);
		}

		float shaderColor = (hsb[2] * 100) / 100F;
		context.setShaderColor(shaderColor, shaderColor, shaderColor, 1);
		context.drawTexture(COLOR_CIRCLE, overlayX + 1, overlayY + 1, 0, 0, 72, 72, 72, 72);
		context.setShaderColor(1, 1, 1, 1);
		...
		if (this.option.isAllowAlpha()) {
			RenderUtils.renderCenteredTextWithMaxWidth(
					context,
					Text.literal(String.valueOf((int) (this.option.getValue().getAlpha() / 255F) * 100)).formatted(Formatting.GRAY),
					13,
					overlayX + 98,
					overlayY + 79,
					-1,
