...
public class EnumDropdownOption<T extends Enum<T>> extends Option<T, EnumDropdownOption<T>> {
	...
	public EnumDropdownOption(Text name, Text description, T value) {
		...
		this.textSupplier = enumValue -> Text.literal(StringUtils.capitalize(enumValue.name().replace('_', ' ').toLowerCase()));
	}
	...
	@Override
	public ConfigOptionEditor<T, EnumDropdownOption<T>> getEditor() {
