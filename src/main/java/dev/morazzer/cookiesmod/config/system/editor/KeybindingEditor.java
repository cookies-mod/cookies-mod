...
public class KeybindingEditor extends ConfigOptionEditor<InputUtil.Key, KeybindingOption> {
	...
	public void render(DrawContext drawContext, int mouseX, int mouseY, float tickDelta, int optionWidth) {
		...
		Text text = this.currentlyEditingKey ? Text.literal("> ").append(keyName).append(" <").formatted(Formatting.YELLOW) : keyName;

		drawContext.drawTexture(BUTTON, optionWidth / 6 - 24, this.getHeight() - 21, 0, 0, 48, 16, 48, 16);
		RenderUtils.renderCenteredTextWithMaxWidth(drawContext, text, 40, (int) (optionWidth / 6f), getHeight() - 13, ~0, true);
	}
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button, int optionWidth) {
		...
		boolean v1 = (mouseY > (this.getHeight() - 21));
		boolean v2 = (mouseY < (this.getHeight() - 5));
